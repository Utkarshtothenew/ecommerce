...
public class CustomerProfileService {
    ...
    public String updateCustomer(CustomerProfileDTO customerProfileDTO,HttpServletRequest request,Boolean ispatch) {

        Customer customer = customerRepo.findByEmail(userEmailFromToken.getUserEmail(request));
        try {
            if (shouldUpdateValue(ispatch,customerProfileDTO.getFirstName())){
                customer.setFirstName(customerProfileDTO.getFirstName());
            }
            if (shouldUpdateValue(ispatch,customerProfileDTO.getLastName())){
                customer.setLastName(customerProfileDTO.getLastName());
            }
            if (shouldUpdateValue(ispatch,customerProfileDTO.getContact())) {
                customer.setContact(customerProfileDTO.getContact());
            }
            if (shouldUpdateValue(ispatch,customerProfileDTO.getImage())) {
    ...
    public boolean shouldUpdateValue(Boolean isPatch,String value){
        return !isPatch || value!=null;
    }
